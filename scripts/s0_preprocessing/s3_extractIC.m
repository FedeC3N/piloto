...
for findex =  1: numel ( dataset )
    ...
    datas   = cat ( 3, trialdata.trial {:} );
    headers = trialdata.hdr;
    ...
    for chindex = 1: numel ( config.channel.groups )
        ...
        label    = ft_channelselection ( channel, headers.label );
        ...
        chanindx = ismember ( headers.label, label );
        label    = headers.label ( chanindx );
