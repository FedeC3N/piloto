...
if ~isfield(cfg,'group')
    ...
    if isempty(cfg.permute)
        if isfield(cfg,'model')
            [~, table] = my_anovan ( data', grupo, 'continuous', cfg.continuous,...
        ...
        s.stat(:,1) = [dummy_stat{:}]';
        s.prob(:,1) = [dummy_prob{:}]';
        s.critval(1) = finv(1-cfg.alpha,table{2,3,1},table{7,3,1});
        ...
        s.stat(:,2) = [dummy_stat{:}]';
        s.prob(:,2) = [dummy_prob{:}]';
        s.critval(2) = finv(1-cfg.alpha,table{3,3,1},table{7,3,1});
        clear dummy_stat dummy_prob
        ...
        s.stat(:,3) = [dummy_stat{:}]';
        s.prob(:,3) = [dummy_prob{:}]';
        s.critval(3) = finv(1-cfg.alpha,table{4,3,1},table{7,3,1});
        clear dummy_stat dummy_prob
        
        % COV
        dummy_stat = squeeze(table(5,6,:));
        dummy_prob = squeeze(table(5,7,:));
        s.stat(:,4) = [dummy_stat{:}]';
        s.prob(:,4) = [dummy_prob{:}]';
        s.critval(4) = finv(1-cfg.alpha,table{5,3,1},table{7,3,1});
        clear dummy_stat dummy_prob
        
        dummy_stat = squeeze(table(6,6,:));
        dummy_prob = squeeze(table(6,7,:));
        s.stat(:,5) = [dummy_stat{:}]';
        s.prob(:,5) = [dummy_prob{:}]';
        s.critval(5) = finv(1-cfg.alpha,table{6,3,1},table{7,3,1});
        clear dummy_stat dummy_prob
        ...
        [~,table,~] = my_anovan(data',{newg1 grupo{2} grupo{3} grupo{4}},...
            'continuous',cfg.continuous, 'model',cfg.model,...
            'display','off');
        dummy_stat = squeeze(table(2,6,:));
        dummy_prob = squeeze(table(2,7,:));
        s.stat(:,1) = [dummy_stat{:}]';
        ...
        [~,table,~] = my_anovan(data',{grupo{1} newg2 grupo{3} grupo{4}},...
            'continuous',cfg.continuous, 'model',cfg.model,...
            'display','off');
        dummy_stat = squeeze(table(3,6,:));
        dummy_prob = squeeze(table(3,7,:));
        ...
        [~,table,~] = my_anovan(data',{newg1 newg2 grupo{3} grupo{4}},...
            'continuous',cfg.continuous, 'model',cfg.model,...
            'display','off');
        dummy_stat = squeeze(table(4,6,:));
        dummy_prob = squeeze(table(4,7,:));
        ...
        % Permutate EDAD
        newg3 = grupo{3};
        newg3 = newg3(randperm(numel(newg3)));        
        [~,table,~] = my_anovan(data',{grupo{1} grupo{2} newg3 grupo{4}},...
            'continuous',cfg.continuous, 'model',cfg.model,...
            'display','off');
        dummy_stat = squeeze(table(5,6,:));
        dummy_prob = squeeze(table(5,7,:));
        ...
        s.prob(:,4) = [dummy_prob{:}]';
        s.critval(4) = finv(1-cfg.alpha,table{5,3,1},table{7,3,1});
        clear newg3 table dummy_stat dummy_prob
        
        % Permutate SEXO
        newg4 = grupo{4};
        newg4 = newg4(randperm(numel(newg4)));        
        [~,table,~] = my_anovan(data',{grupo{1} grupo{2} grupo{3} newg4},...
            'continuous',cfg.continuous, 'model',cfg.model,...
            'display','off');
        dummy_stat = squeeze(table(6,6,:));
        dummy_prob = squeeze(table(6,7,:));
        ...
        s.prob(:,5) = [dummy_prob{:}]';
        s.critval(5) = finv(1-cfg.alpha,table{6,3,1},table{7,3,1});
        clear newg4 table dummy_stat dummy_prob
