function [ clust_out, mass_out ] = find_significant_clusters_template ( Msync1, Msync2, vecinosyvecinas, n, fix, alpha )
...
switch fix
    case 'Condition' % between groups
       [H,P,CI,STATS] = ttest2_mod(Msync1,Msync2,alpha,'both'); %two sample t-test
    case 'Group'% between conditions
       [H,P,CI,STATS] = ttest(Msync1,Msync2,alpha,'both'); %paired t-test
end
...
[cluster, Nclusters] = clustering(t_f_labels, ttest_mat, vecinosyvecinas); 
...
function [cluster, nclusters] = clustering(t_f_labels, ttest_mat, vecinosyvecinas)
...
spatial_labels = vecinosyvecinas;
cluster = zeros(dims);
