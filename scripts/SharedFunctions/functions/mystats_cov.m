function pval = mystats_cov ( data, group, covariates, anova, parametric, nperms )
...
if anova
    ...
    [ pval, table, stats ] = my_anovan ( data, fullgroup, inputs {:} );
    
    % Calculates the pairwise p-value with Tukey's HSD.1
    pairwise  = my_multcompare ( stats, 'display', 'off' );
    pval      = cat ( 1, pval, permute ( pairwise ( :, 6, : ), [ 1 3 2 ] ) );
