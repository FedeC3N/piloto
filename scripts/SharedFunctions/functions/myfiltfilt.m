function data = myfiltfilt ( num, den, data, hilbert )
...
if hilbert
    ...
    f_filter ( 2: floor ( ( nfft + 1 ) / 2 ) ) = 2 * f_filter ( 2: nfft / 2 );
end

% Replicates the filter to match the size of the data.
f_filter = f_filter ( :, ones ( 1, size ( data, 2 ) ) );
...
for index = 1: ceil ( samples / chunksize )
    ...
    chunk    = paddata ( offset + 1: offset + chunklen + 2 * order, : );
    
    % Applies the filter using the FFT.
    f_chunk  = fft ( chunk, nfft, 1 );
%     f_chunk  = bsxfun ( @times, f_chunk, f_filter );
