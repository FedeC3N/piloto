function Y = spm12_extrapolate_def(Y,M)
% Fill in non-finite values in a deformation field
% FORMAT Y = spm12_extrapolate_def(Y,M)
...
% John Ashburner
% $Id: spm12_extrapolate_def.m 6137 2014-08-19 12:43:11Z john $
...
if ~all(msk(:)), % See if needed
    ...
    X          = cat(4,x1,x2,x3);
    M1         = spm12_get_closest_affine(X,Y);
    clear X

    bnd        = spm12_field('boundary'); % For tidying up afterwards
    spm12_field('boundary',1);            % Free boundary conditions
    for d=1:3,
        x          = M1(d,1)*x1 + M1(d,2)*x2 + M1(d,3)*x3 + M1(d,4);
        u          = Y(:,:,:,d) - x; % Displacement field
        u(~msk)    = 0;
        u          = spm12_field(single(msk),u,[vx 0 0.00001 0  1 1]); % Extrapolate displacements
        y          = Y(:,:,:,d);
        y(~msk)    = u(~msk) + x(~msk);
        Y(:,:,:,d) = y;
        clear x y u
    end
    spm12_field('boundary',bnd);
end
