function [ BB, vx ] = spm12_get_bbox ( mri, thr, premul )
% Compute volume's bounding box, for full field of view or object bounds
% FORMAT [BB,vx] = spm12_get_bbox(V, thr)
% V   - mapped image volume(s) (from spm12_vol) or filename (empty for GUI)
...
% Ged Ridgway
% $Id: spm12_get_bbox.m 5398 2013-04-12 12:37:00Z ged $
...
% thr = 'old' - reproduce spm12_write_sn/bbvox_from_V (and elsewhere)
% premul      - a matrix that premultiplies V.mat, as used in spm12_orthviews

%-Compute voxel dimensions (for compatibility with bbvox_from_V)
%--------------------------------------------------------------------------
P = spm_imatrix(mri.transform);
vx = P(7:9);
...
% Note that spm12_imatrix(diag([-1 1 1 1])) is [-1 1 1] as expected.
...
if nargin < 2 || isempty(thr) || strcmpi(thr, 'fv')
    ...
    XYZ = mri.transform * corners;
elseif strcmpi(thr, 'old')
    % code from spm12_write_sn/bbvox_from_V (and other places)
    % NB: main difference is that vx(1)<0 gives descending BB(:,1),
    % shouldn't be used if V.mat contains rotations or skews.
    o  = mri.transform \ [ 0 0 0 1 ]';
    o  = o(1:3)';
    BB = cat ( 1, -vx .* ( o - 1 ), vx .* ( mri.dim ( 1: 3 ) - o ) );
    if exist('premul', 'var')
        warning('spm12_get_bbox:old_and_premul', 'old method ignores premul')
    end
else
    % image-based bounding box using voxel intensities
    img = spm12_read_vols(V);
    if ischar(thr)
        switch lower(thr)
    ...
    if nnz(img) == 0
        warning('spm12_get_bbox:nothing', ...
            'Threshold leaves no voxels, returning full field of view');
        if exist('premul', 'var')
            [BB,vx] = spm12_get_bbox(V, 'fv', premul);
        else
            [BB,vx] = spm12_get_bbox(V, 'fv');
        end
        return
    else
