function results = spm12_preproc8(obj)
...
for z=1:length(z0)
   ...
   %y1  = M(2,1)*x0 + M(2,2)*y0 + (M(2,3)*z0(z) + M(2,4));
    ...
    if isfield(obj,'msk') && ~isempty(obj.msk)
        ...
        msk        = spm12_sample_vol(VM,x0,y0,o*z0(z),0);
...
for iter=1:30
    ...
    for z=1:length(z0)
        ...
        b          = spm12_sample_priors8(tpm,x1,y1,z1);
        for k1=1:Kb
            buf(z).dat(:,k1) = b{k1};
        end
    end
    ...
    for iter1=1:8
        if use_mog
            ...
            for subit=1:20
                ...
                ll   = llr+llrb;
                for n=1:N
                    chan(n).lik  = spm12_smohist(chan(n).hist,chan(n).lam);
    ...
    for subit=1:3
        ...
        for z=1:length(z0)
            ...
            [b,db1,db2,db3] = spm12_sample_priors8(tpm,x1,y1,z1);
            clear x1 y1 z1
