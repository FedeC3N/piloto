function [cls,M1] = spm12_preproc_write8(res,tc,bf,df,mrf,cleanup,bb,vx)
...
[bb1,vx1] = spm12_get_bbox8(tpm.V(1), 'old');
...
for n=1:N,
    ...
    if bf(n,2),
        chan(n).Nc      = nifti;
        chan(n).Nc.dat  = file_array(fullfile(pth1,['m_', nam1, '.nii']),...
                                     res.image(n).dim(1:3),...
    ...
    if bf(n,1),
        chan(n).Nf      = nifti;
        chan(n).Nf.dat  = file_array(fullfile(pth1,['bf_', nam1, '.nii']),...
                                     res.image(n).dim(1:3),...
                                     [spm_type('float32') spm_platform('bigend')],...
                                     0,1,0);
...
for k1=1:Kb,
    ...
    if tc(k1,1),
        tiss(k1).Nt      = nifti;
        tiss(k1).Nt.dat  = file_array(fullfile(pth,['c', num2str(k1), '_', nam, '.nii']),...
                                      res.image(n).dim(1:3),...
    ...
    if do_defs,
        ...
        if do_cls,
            ...
            if isfield(res,'mg'),
                ...
                b   = spm12_sample_priors8(tpm,t1,t2,t3);
                wp  = res.wp;
                s   = zeros(size(b{1}));
                for k1 = 1:Kb,
                    b{k1} = wp(k1)*b{k1};
...
if any(tc(:,2)),
    ...
    [M,R]  = spm12_get_closest_affine(x,y1,single(cls{ind(1)})/255);
    clear x y1
    ...
    for k1=1:size(tc,1),
        if tc(k1,2),
            ...
            Ni      = nifti;
            Ni.dat  = file_array(fullfile(pth,['rc', num2str(k1), '_', nam, '.nii']),...
                                 odim,...
                                 [spm_type('float32') spm_platform('bigend')],...
                                 0,1,0);
            Ni.mat         = mat;
...
if any(tc(:,3)) || any(tc(:,4)) || nargout>=1,
    ...
    for k1 = 1:Kb,
        if ~isempty(cls{k1}),
            c = single(cls{k1})/255;
            if any(tc(:,3)),
                [c,w]       = spm12_diffeo('push',c,y,d1(1:3));
                vx          = sqrt(sum(M1(1:3,1:3).^2));
                spm12_field('boundary',1);
                C(:,:,:,k1) = spm12_field(w,c,[vx  1e-6 1e-4 0  3 2]);
                clear w
            else
                c      = spm12_diffeo('push',c,y,d1(1:3));
            end
            if nargout>=1,
                cls{k1} = c;
            end
            if tc(k1,4),
                N      = nifti;
                N.dat  = file_array(fullfile(pth,['mwc', num2str(k1), '_', nam, '.nii']),...
                                    d1,...
                                    [spm_type('float32') spm_platform('bigend')],...
                                    0,1,0);
                N.mat  = M1;
    ...
    if any(tc(:,3)),
        ...
        for k1=1:Kb,
            if tc(k1,3),
                N      = nifti;
                N.dat  = file_array(fullfile(pth,['wc', num2str(k1), '_', nam, '.nii']),...
                                    d1,'uint8',0,1/255,0);
                N.mat  = M1;
                N.mat0 = M1;
...
if df(2),
    y         = spm12_diffeo('invdef',y,d1,eye(4),M0);
    y         = spm12_extrapolate_def(y,M1);
    N         = nifti;
