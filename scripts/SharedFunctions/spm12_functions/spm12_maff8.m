function [M,ll,h] = spm12_maff8(varargin)
...
x2 = x{2};
x3 = x{3};
[mu,isig] = spm12_affine_priors(regtyp);
mu        = [zeros(6,1) ; mu];
...
for iter=1:200
    ...
    for search = 1:nsearch,
       ...
       %drawnow
        ...
        for i=1:length(x3)
            ...
            y2    = y2(msk);
            y3    = y3(msk);
            b     = spm12_sample_priors8(tpm,y1,y2,y3);
            buf(i).b    = b;
            buf(i).msk1 = msk;
       ...
       %spm12_plot_convergence('Set',ll1);
    ...
    for i=1:length(x3)   % Loop over slices
        ...
        if nz
            ...
            dmi2  = zeros(nz,1);
            dmi3  = zeros(nz,1);
            [b, db1, db2, db3] = spm12_sample_priors8(tpm,y1,y2,y3);
