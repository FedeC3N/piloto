...
% Makes sure that all the electrodes has digitalization to avoid warning.
% Gets the number of recorded and digitalized EEG channels.
digEEG = sum ( cat ( 1, dig.kind ) == 3 );
recEEG = sum ( cat ( 1, chs.kind ) == 2 );

% Adds new channels if required.
if recEEG > numel ( digEEG )
    for cindex = numel ( digEEG ) + 1: recEEG
        iindex = numel ( dig ) + 1;
        dig ( iindex ).kind        = 3;
        dig ( iindex ).ident       = cindex;
        dig ( iindex ).r           = nan ( 1, 3 );
        dig ( iindex ).coord_frame = dig ( find ( cat ( 1, dig.kind ) == 3, 1 ) ).coord_frame;
    end
end
