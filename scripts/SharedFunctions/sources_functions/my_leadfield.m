...
switch headmodel.type
    ...
    case 'localconcentricspheres'
        ...
        for cindex = 1: nsens
            ...
            leadfield ( cindex, : ) = mymsc_leadfield ( grid.pos ( grid.inside, : ), sens.elecpos ( cindex, : ), sensmodel, moveinside );
        end
    ...
    case 'concentricspheres'
        ...
        leadfield = mymsc_leadfield ( grid.pos ( grid.inside, : ), sens.elecpos, headmodel, moveinside );
        
    % Single shell.
    case 'singleshell'
        
        % Calculates the leadfield.
        leadfield = myss_leadfield ( grid, headmodel, sens );
        
    % OpenMEEG.
    case { 'bem3' 'openmeeg' }
        
        % Calculates the leadfield.
        leadfield = myom_leadfield ( cfg.headmodel, cfg.grid, sens );
        
    % OpenMEEG.
    case { 'mne' }
        
        % Calculates the leadfield.
        leadfield = mymne_leadfield ( cfg.headmodel, cfg.grid, sens, cfg.header );
        
    % Otherwise relies on FieldTrip.
    otherwise
        
        % Calculates the leadfield using FieldTrip.
        leadfield = ft_compute_leadfield ( grid.pos ( grid.inside, : ), sens, headmodel, 'reducerank', 'no' );
%         grid      = ft_prepare_leadfield ( cfg, data );
%         return
end
