...
if ~isfield ( headmodel, 'hm' ) && ~isfield ( headmodel, 'hm_dsm' )
    fprintf ( 1, 'Head model matrix not present in the data. Calculating it.\n' );
    headmodel = myom_headmodel ( headmodel );
end

% Transforms the stored head model structure to a symmetric matrix.
if isfield ( headmodel, 'hm' ) && isstruct ( headmodel.hm )
...
if ~isfield ( headmodel, 'dsm' ) && ~isfield ( headmodel, 'hm_dsm' )
    fprintf ( 1, 'Source matrix not present in the data. Calculating it.\n' );
...
% If not present, calculates inv ( hm ) * dsm.
